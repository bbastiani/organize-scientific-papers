...
try:
  ...
  parse_xml = lambda s: etree.XML(s, etree.XMLParser(recover=True))
except ImportError:
  import xml.etree.ElementTree as etree
  parse_xml = lambda s: etree.XML(s)
...
def convert_pdf_to_xml(path):
  ...
  cmd = [dir + '/poppler/pdftohtml.exe', '-xml', '-f', '1', '-l', '1', '-i', '-q', '-nodrm', '-hidden', '-stdout', path]
  xml_string = subprocess.check_output(cmd, stderr=open(os.devnull, 'w'))
  return parse_xml(remove_control_chars(xml_string.decode("utf-8")).encode("utf-8"))
...
def format_ligatures(title, _config):
  ...
  return title.decode("utf-8").replace('ﬁ', 'fi') \
              .replace('ﬂ', 'fl')
...
def main(args):
  ...
  unformatted string."""
  # try:
  title = extract_title(args.file, args)

  if not title:
    return 1, 'Error: No title found'

  print(title)

  if args.rename:
    filename = '{}.pdf'.format(sanitize_filename(title))
    filepath = os.path.join(os.path.dirname(args.file), filename)
    os.rename(args.file, filepath)
...
if __name__ == '__main__':
  try:
    ...
    parser.add_argument('file', metavar='FILE', type=filepath, help='Path to PDF file')
    parser.add_argument('-r', '--rename', action='store_true', help='Rename file with found title')
